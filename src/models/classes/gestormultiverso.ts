...
import { IntDimensiones, IntPersonajes, IntEspecies, IntLocalizacion, IntArtefactos, Evento } from '../interfaces.js';
...
export class GestorMultiverso {
   ...
   */
  public async updateDimension(id: string, dimensionActualizada: IntDimensiones): Promise<void> {
    ...
    if (id !== dimensionActualizada.id) {
      if(this.data.dimensiones.some(d => d.id === dimensionActualizada.id)) {
        throw new Error(`El nuevo ID '${dimensionActualizada.id}' ya esta en uso`);
      }
  ...
  public async addArtifact(newArtifact: IntArtefactos): Promise<void> {
    ...
    if (!this.data.personajes.some(p => p.id === newArtifact.inventor.id)) {
      throw new Error(`No se puede añadir: El inventor '${newArtifact.inventor.nombre}' no esta registrado`);
    }
   ...
   */
  public async updateArtifact(id: string, updatedArtifact: IntArtefactos): Promise<void> {
    const index = this.data.artefactos.findIndex(a => a.id === id);
    if (index === -1) throw new Error(`Artefacto '${id}' no encontrado`);

    if (id !== updatedArtifact.id && this.data.artefactos.some(a => a.id === updatedArtifact.id)) {
  ...
  public async addCharacter(newCharacter: IntPersonajes): Promise<void> {
    ...
    if (!this.data.dimensiones.some(d => d.id === newCharacter.dim_origen.id)) {
      throw new Error(`Error: La dimension '${newCharacter.dim_origen.nombre}' no esta registrada`);
    }
    this.data.personajes.push(newCharacter as Personaje);
    await almacen.write();
  }

  public async updateCharacter(id: string, updatedCharacter: IntPersonajes): Promise<void> {
    const index = this.data.personajes.findIndex(l => l.id === id);
    if (index === -1) throw new Error(`Personaje '${id}' no encontrada`);

    if (id !== updatedCharacter.id && this.data.personajes.some(l => l.id === updatedCharacter.id)) {
  ...
  public async addLocation(newLocation: IntLocalizacion): Promise<void> {
    ...
    if (!this.data.dimensiones.some(d => d.id === newLocation.dimension.id)) {
      throw new Error(`Error: La dimension '${newLocation.dimension.nombre}' no existe en el registro`);
    }
    this.data.localizaciones.push(newLocation as Localizacion);
    await almacen.write();
  }

  public async updateLocation(id: string, updatedLocation: IntLocalizacion): Promise<void> {
    const index = this.data.localizaciones.findIndex(l => l.id === id);
    if (index === -1) throw new Error(`Localizacion '${id}' no encontrada`);

    if (id !== updatedLocation.id && this.data.localizaciones.some(l => l.id === updatedLocation.id)) {
   ...
   */
  public async registrarViaje( IdPersonaje: string, IdDestino: string, motivo: string ): Promise<void> {
    ...
    if (!destino) throw new Error(`La dimension destino '${IdDestino}' no existe`);

    const origenId = personaje.dim_origen.id;
    personaje.localizacion = destino;

    almacen.data.regEventos.push({fecha: new Date(), descripcion: `Viaje registrado: ${personaje.nombre} ha saltado de ${ this.data.dimensiones.find(d => d.id === origenId)!.nombre } a ${destino.nombre}`});
    await almacen.write();
  }
  ...
  public async updateSpecies(id: string, updatedSpecies: IntEspecies): Promise<void> {
    const index = this.data.especies.findIndex(e => e.id === id);
    if (index === -1) throw new Error(`Especie '${id}' no encontrada.`);
    if (id !== updatedSpecies.id && this.data.especies.some(e => e.id === updatedSpecies.id)) {
  ...
  public async deleteSpecies(id: string): Promise<void> {
    ...
    const personajesAfectados = this.data.personajes.filter(p => p.especie.id === id);
    if (personajesAfectados.length > 0) {
      console.warn(`AVISO: Estas eliminando la especie '${id}', pero hay ${personajesAfectados.length} personajes que pertenecen a ella.`);
    }
