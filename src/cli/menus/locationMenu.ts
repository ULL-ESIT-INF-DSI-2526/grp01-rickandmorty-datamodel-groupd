...
/**
 * Función que muestra el menú de gestión de localizaciones y
 * maneja las interacciones del usuario para añadir, modificar, eliminar, listar y buscar localizaciones en el sistema de gestión del
 * multiverso de Rick and Morty.
 * @param gestor - Instancia del GestorMultiverso que se utiliza para realizar las operaciones sobre las localizaciones
 * @returns void
 * @throws Error si ocurre algún problema durante las operaciones de gestión de localizaciones (ej: ID no encontrado, dimensión no encontrada, etc.)
 */
  ...
  while (!back) {
    ...
    switch (response.option) {
      case 'add':
        ...
        const dimension = gestor
          .getAllDimensions()
          .find(
            (d) =>
              d.nombre === newLocation.dimension ||
              d.id === newLocation.dimension,
          );
        if (!dimension) {
          console.log(
            'Dimensión no encontrada. Usa nombre o ID de una dimensión existente.',
          );
          break;
        }
        await gestor.addLocation(
          new Localizacion(
            newLocation.id,
            newLocation.nombre,
            newLocation.tipo,
      ...
      case 'edit':
        ...
        const currentLocation = gestor
          .getAllLocations()
          .find((l) => l.id === updatedLocation.id);
        if (!currentLocation) {
        ...
        const updatedDimension = updatedLocation.dimension
          ? gestor
              ...
              .find(
                (d) =>
                  d.nombre === updatedLocation.dimension ||
                  d.id === updatedLocation.dimension,
              )
        ...
        const locationToSave: IntLocalizacion = {
          ...
          poblacion_aprox:
            updatedLocation.poblacion_aprox ?? currentLocation.poblacion_aprox,
          descripcion:
            updatedLocation.descripcion ?? currentLocation.descripcion,
        };
