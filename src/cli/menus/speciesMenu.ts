...
/**
 * Función que muestra el menú de gestión de especies en la CLI. Permite al usuario
 * añadir, modificar, eliminar y listar especies registradas en el sistema.
 * @param gestor - Instancia del GestorMultiverso que maneja la lógica de negocio relacionada con las especies
 * El menú se muestra en un bucle hasta que el usuario decide volver al menú principal.
 * Cada opción del menú llama a una función específica para manejar la acción correspondiente, como agregar una nueva especie,
 * editar una existente, eliminar una especie o listar todas las especies registradas.
 */
  ...
  while (!back) {
    ...
    switch (response.option) {
      case 'add':
        ...
        const origenDimension = gestor
          .getAllDimensions()
          .find((d) => d.nombre === newSpeciesInput.origen);
        const origenLocalizacion = gestor
          .getAllLocations()
          .find((l) => l.nombre === newSpeciesInput.origen);
        const origen = origenDimension || origenLocalizacion;
        if (!origen) {
          console.log('Origen (dimensión o localización) no encontrado.');
          break;
        }
        await gestor.addSpecies(
          new Especies(
            newSpeciesInput.id,
      ...
      case 'edit':
        ...
        const currentSpecies = gestor
          .getAllSpecies()
          .find((s) => s.id === editSpeciesInput.id);
        if (!currentSpecies) {
        ...
        const updatedOrigenDimension = editSpeciesInput.origen
          ? gestor
              .getAllDimensions()
              .find((d) => d.nombre === editSpeciesInput.origen)
          : currentSpecies.origen;
        const updatedOrigenLocalizacion =
          editSpeciesInput.origen && !updatedOrigenDimension
            ? gestor
                .getAllLocations()
                .find((l) => l.nombre === editSpeciesInput.origen)
            : undefined;
        const updatedOrigen =
          updatedOrigenDimension ||
          updatedOrigenLocalizacion ||
          currentSpecies.origen;
        const updatedSpecies: IntEspecies = {
          ...currentSpecies,
          nombre: editSpeciesInput.nombre ?? currentSpecies.nombre,
          descripcion:
            editSpeciesInput.descripcion ?? currentSpecies.descripcion,
          origen: updatedOrigen,
          tipo: editSpeciesInput.tipo ?? currentSpecies.tipo,
          esperanza_vida:
            editSpeciesInput.esperanza_vida ?? currentSpecies.esperanza_vida,
        };
