...
/**
 * Función que muestra el menú de gestión de dimensiones y maneja las interacciones
 * del usuario para añadir, modificar, eliminar y listar dimensiones en el sistema de gestión del multiverso de Rick and Morty.
 * @param gestor - Instancia de GestorMultiverso que se utiliza para realizar las operaciones de gestión de dimensiones
 * @returns void
 * @throws Error si ocurre algún problema durante las operaciones de gestión de dimensiones
 */
  ...
  while (!back) {
    ...
    switch (response.option) {
      ...
      case 'edit':
        ...
        const currentDimension = gestor
          .getAllDimensions()
          .find((d) => d.id === editDimensionInput.id);
        if (!currentDimension) {
        ...
        const updatedDimension: IntDimensiones = {
          ...
          nivel_tec: editDimensionInput.nivel_tec
            ? new Nivel(editDimensionInput.nivel_tec)
            : currentDimension.nivel_tec,
          descripcion:
            editDimensionInput.descripcion ?? currentDimension.descripcion,
        };
