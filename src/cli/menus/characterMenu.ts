...
/**
 * Muestra el menú de gestión de personajes en la CLI.
 * @param gestor - Instancia del GestorMultiverso para interactuar con los datos de personajes y otras entidades relacionadas.
 * El menú permite al usuario añadir, eliminar, modificar, buscar y listar personajes, así como localizar versiones alternativas de un personaje dado su nombre.
  ...
  while (!back) {
    ...
    switch (response.option) {
      case 'add':
        ...
        const newSpecies = gestor
          .getAllSpecies()
          .find((s) => s.nombre === newCharacter.species);
        const newDimension = gestor
          .getAllDimensions()
          .find((d) => d.nombre === newCharacter.dimension);
        if (!newSpecies || !newDimension) {
          console.log(
            'Especie o dimensión no encontrada. Deben existir antes de crear un personaje.',
          );
          break;
        }
        await gestor.addCharacter(
          new Personaje(
            newCharacter.id,
            newCharacter.name,
            newSpecies,
      ...
      case 'alternatives':
        ...
        const alternatives: IntPersonajes[] =
          gestor.localizarVersionesAlternativas(alternativeName.value);
        if (alternatives.length === 0) {
          console.log(
            'No se han encontrado versiones alternativas para ese personaje.',
          );
          break;
        }
      ...
      case 'modify':
        ...
        const updatedDimension = character.dimension
          ? gestor
              .getAllDimensions()
              .find((d) => d.nombre === character.dimension)
          : currentCharacter.dim_origen;
        if (!updatedSpecies || !updatedDimension) {
          console.log(
            'Especie o dimensión inválida para actualizar el personaje.',
          );
          break;
        }
        const updatedCharacter: IntPersonajes = {
          ...
          nivel_inteligencia: new Nivel(
            character.intelligenceLevel ??
              nivelValue(currentCharacter.nivel_inteligencia),
          ),
